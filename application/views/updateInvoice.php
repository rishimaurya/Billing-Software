...
<html>
 ...
 <body style="font-size:1.4rem;">
   <div class="container">
    ...
    <form class="form-horizontal" method="POST" action="<?php echo base_url('Invoice/submitUpdateInvoice'); ?>">
    <div class="form-group row">
        <div class="col-sm-2">
			<label for="client">Client Name:</label>
			<input type="text" class="form-control" id="client" name="client" value="<?php echo ucwords($iv[0]->client_name); ?>"/>
		</div>
	
	    <div class="col-sm-2">
			<label for="obile">Mobile :</label>
			<input type="number" class="form-control col-xs-2" id="mobile" value="<?php echo $iv[0]->client_mobile; ?>" name="mobile" maxlength="10" required/>
		</div>
		
        <div class="col-sm-2">
			<label for="invoicen">Invoice No.:</label>
			<input type="text" class="form-control col-xs-2" id="invoicen" name="invoicen" tabIndex="-1" readonly="true" value="<?php echo $iv[0]->sale_id; ?>" />
		</div>
	
        <div class="col-sm-2">
			<label for="IDate">Invoice Date:</label>
			<input type="date" class="form-control col-xs-2" id="invoice_date" name="invoice_date" value="<?php echo $iv[0]->sale_date; ?>" />
		</div>
	
        <div class="col-sm-2">
			<label for="IDate">Net-Total:</label>
			<input type="number" class="form-control col-xs-2" id="netTotal" name="netTotal" tabIndex="-1" readonly="true" value="<?php echo $iv[0]->total; ?>" />
		</div>
	</div>
...
<div class="form-group row">
		...
		<div class="col-sm-2">
			...
			<input type="text" class="form-control col-xs-2" id="sprofit" name="sprofit" readonly="true" value="<?php echo $iv[0]->profit; ?>"/>
		</div>
		
		<div class="col-sm-2">
			<label for="invoicen"> Customer Discount :</label>
			<input type="text" class="form-control col-xs-2" id="cdiscount" name="cdiscount" readonly="true" value="<?php echo $iv[0]->discount; ?>" />
		</div>
...
<!--   <div class="form-group">
  ...
  <table id="mytable" class="table table-bordered">
    ...
    <tbody>
        <?php
		$i=1;foreach($item as $d)
		  { 
		?>
      <tr id="<?php echo $i; ?>">
        <td><span style="color:blue" onclick="removeRow(<?php echo $i; ?>)"><img src="<?php echo base_url('d3.png'); ?>" /></span></td>
        <td><input type="text" class="form-control" id="br_<?php echo $i; ?>" name="br[]" 	  onkeyup="searchBarcode(<?php echo $i; ?>,this.value)" autofocus="true" required="true" value="<?php echo $d->product_id; ?>" /></td>
        <td><input type="text"   class="form-control" id="im_<?php echo $i; ?>" name="itemName[]" tabIndex="-1"  required="true" value="<?php echo $d->description; ?>" /></td>
        <td><input type="tel" class="form-control" id="qt_<?php echo $i; ?>" name="quantity[]" onkeyup="calculateSb(1)" onchange="calculateSb(<?php echo $i; ?>)"   tabIndex="-1" min="0" max="<?php echo $d->units+$d->quantity; ?>" value="<?php echo $qua = $d->quantity; ?>"  required="true" /></td>
                <input type="hidden" id="qth_<?php echo $i; ?>" name="quant[]" value="<?php echo $d->quantity; ?>"/>
        <td><input type="number" class="form-control" id="up_<?php echo $i; ?>" name="uPrice[]"   onkeyup="calculateSb(1)" onchange="calculateSb(<?php echo $i; ?>)" tabIndex="-1" min="1" value="<?php echo $sp = $d->price; ?>" required="true" /></td>
        <td><input type="number" class="form-control" id="st_<?php echo $i; ?>" name="subTotal[]" tabIndex="-1" value="<?php echo $d->quantity*$d->price; ?>" /></td>
      
              <input type="hidden" id="mrp_<?php echo $i; ?>" name="mrp[]" value="<?php echo $mrp = $d->inv_mrp;?>"/>
              <input type="hidden" id="cp_<?php echo $i; ?>" name="cPrice[]" value="<?php echo $cp = $d->costprice;?>"/>
              <input type="hidden" id="profit_<?php echo $i; ?>" name="prof[]" value="<?php echo $qua*($sp - $cp); ?>""/>
              <input type="hidden" id="custD_<?php echo $i; ?>" name="custD[]" value="<?php echo $qua*($d->inv_mrp - $d->costprice);?>"/>
      </tr>
      <?php
           $i++;
	     }
 ...
 <script>
	 ...
	 function addRow()
	 {
	   ...
	   $('#mytable tbody').append('<tr id="'+j+'"><td><span style="color:blue" onclick="removeRow('+j+')"><img src="<?php echo base_url('d3.png'); ?>" /></span></td>\
	                                ...
	                                <td><input type="tel" class="form-control" id="qt_'+j+'" name="quantity[]" onkeyup="calculateSb('+j+')" onchange="calculateSb('+j+')" tabIndex="-1" min="1" required="true" required="true" /></td>\
	                                <td><input type="number" class="form-control" id="up_'+j+'" name="uPrice[]"   onkeyup="calculateSb('+j+')" onchange="calculateSb('+j+')" tabIndex="-1" min="1" required="true" /></td>\
	                                <td><input type="number" class="form-control" id="st_'+j+'" name="subTotal[]" tabIndex="-1" required="true" /></td>\
